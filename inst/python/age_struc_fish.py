...
class AsmEnv(gym.Env):
    def __init__(self, render_mode: Optional[str] = 'rgb_array', config={}):
        ...
        #
        # functions
        self.harvest = config.get('harvest', default_harv)
        self.dynamics = config.get("dynamics", default_dyn)
        self.observe = config.get("observe", default_obs)
        self.utility = config.get("utility", default_util)
        self.get_r_devs = config.get('get_r_devs', default_r_devs)
    ...
    def get_exploitation_rate(self, action):
        """
        input: action in [-1, +1]
        output: exploitation rate in [0, 1]
        """
        action = np.clip(action, [-1], [1])
        exploitation_rate = (action + 1.0) / 2.0 
